...
Period_statistics_t audioPeriodTimer;
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	perror("Audio queue full; Unable to queue sound");
	pthread_mutex_unlock(&audioMutex);
}
...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	for(int i = 0; i < MAX_SOUND_BITES; i++){
		...
		if(soundBites[i].pSound != NULL){
			...
			//how far into the sound we are
			int offset = soundBites[i].location;
			int soundBiteSize = soundBites[i].pSound->numSamples;
			int sample = 0;

			//process soundbite
			for(int j = 0; j < size; j++){
				
				//break the loop if we have gone through every sample of the 
				if(offset >= soundBiteSize){
					//erase the sound from the queue once it's done playing
					soundBites[i].pSound = NULL;
					break;
				}

				sample = soundBites[i].pSound->pData[offset];
				int newVal = buff[j] + sample;
			...
			//increment location in soundbite
